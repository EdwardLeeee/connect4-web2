...
**設計稿**：在 19b9133 上截圖，也就是 #63 合併後、#64 之前的版本。所以設計稿不含 #64 的「離開」上移和對手棋色；對局畫面這兩點以現在的 main 為準，比對時只看本輪 01–04 的變化。
...
- **03 離線時的玩家設定：只鎖暱稱，語言照常可改**（網站和 app 都一樣；`R23-03-離線改暱稱-C-只鎖暱稱語言可改-desktop與mobile.png`）
  ...
  - **語言可以切換**：
    ...
    - 暱稱欄位的內容不送出。
    - 這一段只要 front 實作：語言先存在本機、立刻套用；連線後把裝置上記住的暱稱加上新語言，用現有的 `PATCH /api/session` 送出。
...
- **04b 沒網路時打開 app：記住上次的暱稱**（`R23-04b-離線開app的暱稱-A-記住上次的暱稱-mobile.png`）
  ...
  - **改成**：app 把上次連線時的暱稱和語言存在手機上，沒網路打開時照樣使用：
    ...
    - 本機 AI 局上自己的名字。
  - **全新安裝、從沒連過線時：手機先產生預設暱稱**（第 24 輪，使用者 2026-09-26 選 C；`R24-01-從沒連線時自己的名字-C-手機先產生玩家編號-mobile與desktop.png`）
    - app 第一次打開就在手機上產生預設暱稱，格式和伺服器相同，依當時的介面語言（見 05）：中文「玩家 NNNN」，英文「Player NNNN」。NNNN 是 1000–9999 的隨機數字。
    - 頭像顯示這個名字的第一個字（「玩」／「P」）；玩家設定和本機 AI 局的對局卡也都用這個名字。
    - 第一次連上伺服器時，用 `PATCH /api/session` 把這個暱稱和語言存上去，之後網站和 app 都是同一個名字。
    - 這條取代原本「從沒連過線時頭像維持『?』」的規則，所以 app 不會再出現沒有名字的狀態。
    - 沒被選的方案：A 顯示「你」、B 顯示「玩家」、留空。
...
- **05 預設暱稱依介面語言（網站和 app 都一樣；使用者 2026-09-26）**
  - 預設暱稱改成依語言產生：中文「玩家 NNNN」，英文「Player NNNN」。
    - 伺服器現在不分語言，一律給「玩家 NNNN」，所以 **back 要調整**。
    - 預設暱稱的格式用新文案 `session.defaultNickname`。
  - **還沒自己改過名字的人**切換語言時，預設暱稱跟著換成另一種語言的版本，數字不變（「玩家 4821」↔「Player 4821」）。
    - 離線時切換語言也一樣，連線後一起同步。
  - **自己取過名字的人**，切換語言時名字不動。
    - 使用者自己取的名字剛好長得像「玩家 1234」時，也不能被當成預設名字改掉。
    - 怎麼記住「還是預設名字」由 back 和 front 決定，例如存一個旗標。
- **06 伺服器重啟後自動補回暱稱和語言（網站和 app 都一樣；使用者 2026-09-26）**
  - 伺服器重啟（例如每次部署）後拿到新的 session 時，網站和 app 都自動把裝置上記住的暱稱和語言補回去。
    - 用一次 `PATCH /api/session` 同時送出暱稱和語言。
    - 畫面不能閃出伺服器新給的預設暱稱或預設語言。
  - 只在新建立的 session 上補回，不能蓋掉伺服器在同一個 session 上的改動。
  - 網站因此會在瀏覽器裡記住暱稱和語言；現有的 cookie 流程不變。
- **沒被選的方案**：
  - 01：文案 A、B、C。
...
| `session.defaultNickname` | 玩家 {n} | Player {n} | 新增（3.2.0 05）：預設暱稱，伺服器和 app 都用；{n} 是 1000–9999 |
