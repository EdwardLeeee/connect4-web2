...
# docs/mobile-release.md lists what to do on Apple's websites between the steps.
#
# Usage:
#   mobile/scripts/ios-signing.sh csr
#       make the private key and a certificate signing request to upload to Apple
...
#       keep copies next to the key and store everything as GitHub secrets
#
...
case "$1" in
    csr)
        ...
        openssl req -new -newkey rsa:2048 -nodes -keyout "${DIR}/distribution.key" \
            -out "${DIR}/distribution.csr" -subj "/CN=Four In A Row Distribution/C=TW"
        echo "upload ${DIR}/distribution.csr as an Apple Distribution certificate request"
        ;;
    p12)
        cer="${2:-}"
        [ -f "${cer}" ] || usage
        openssl x509 -inform DER -in "${cer}" -out "${DIR}/distribution.pem"
        cp "${cer}" "${DIR}/distribution.cer"
    ...
    secrets)
        ...
        cp "${profile}" "${DIR}/app-store.mobileprovision"
        cp "${p8}" "${DIR}/$(basename "${p8}")"
        ...
        base64 -w0 "${p8}" | gh secret set ASC_API_KEY_P8_BASE64 --repo "${REPO}"
