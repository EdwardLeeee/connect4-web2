# 隱私權政策／Privacy Policy

適用於「四子棋」app（英文名稱 Four In A Row）與 https://connect4.oraclelee.com 。
Applies to the Four In A Row app (四子棋) and https://connect4.oraclelee.com.

最後更新／Last updated: 2026-09-26

## 中文

- 不需要帳號。我們不收集姓名、email、電話、位置等個人資料；沒有廣告，也沒有分析工具。
- 遊戲只存你設定的暱稱與語言，以及一組隨機產生、用來辨識連線的代碼。這些資料放在伺服器記憶體，
  伺服器重啟就清除。暱稱會顯示給同一局的對手。網站用 cookie 記住這組連線代碼（最長 30 天）；
  app 則把連線代碼和上次的暱稱與語言存在手機上，沒有網路時也能顯示。
- app 的 AI 對局在手機上進行，對局進度與比分只存在手機上，不會傳到我們的伺服器；離開對局或刪除 app
  就會清除。
- 跟一般網站一樣，伺服器與網路服務商 Cloudflare 會留有連線紀錄（例如 IP 位址、時間、瀏覽器資訊），
  只用於維運與安全，不用來追蹤或投放廣告，也不提供給其他人；紀錄會隨伺服器的紀錄輪替自動刪除。
  Cloudflare 依它自己的[隱私權政策](https://www.cloudflare.com/privacypolicy/)處理。
- 刪除：刪掉 app 就會清掉手機上的連線代碼、暱稱、語言與 AI 對局進度；伺服器上的遊戲資料在重啟時清除。
- 聯絡：請到本專案的 [GitHub Issues](https://github.com/EdwardLeeee/connect4-web2/issues) 留言。

## English

- No account is needed. We do not collect personal data such as your name, email address, phone
  number or location. There are no ads and no analytics.
- The game keeps only the nickname and language you choose, plus a randomly generated code that
  identifies your connection. They live in the server's memory and are erased whenever the server
  restarts. Your nickname is shown to your opponent in the same game. The website remembers the
  connection code in a cookie (for up to 30 days); the app stores the connection code and your
  last nickname and language on your phone, so they still show without a connection.
- In the app, games against the AI run on your phone. Their progress and score stay on your phone
  and are never sent to our server; they are erased when you leave the game or delete the app.
- Like any website, the server and our network provider Cloudflare keep connection logs (for example
  IP address, time and browser details). They are used only to run and secure the service, never
  for tracking or advertising, are not given to anyone else, and are deleted automatically as the
  server's logs rotate. Cloudflare handles them under its own
  [privacy policy](https://www.cloudflare.com/privacypolicy/).
- Deleting data: deleting the app removes the connection code, nickname, language and AI game
  progress from your phone; game data on the server is erased when it restarts.
- Contact: open an issue on this project's
  [GitHub Issues](https://github.com/EdwardLeeee/connect4-web2/issues).
