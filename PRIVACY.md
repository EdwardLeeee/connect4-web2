...
適用於「四子棋」app（英文名稱 Four In A Row）與 https://connect4.oraclelee.com 。
Applies to the Four In A Row app (四子棋) and https://connect4.oraclelee.com.
...
- 遊戲只存你設定的暱稱與語言，以及一組隨機產生、用來辨識連線的代碼。這些資料放在伺服器記憶體，
  ...
  並在瀏覽器裡記住你的暱稱與語言；app 則把連線代碼和上次的暱稱與語言存在手機上，沒有網路時也能顯示。
  伺服器重啟後，網站和 app 會自動補回你上次的暱稱與語言。
- app 的 AI 對局在手機上進行，對局進度與比分只存在手機上，不會傳到我們的伺服器；離開對局或刪除 app
  就會清除。
...
- No account is needed. We do not collect personal data such as your name, email address, phone
...
- The game keeps only the nickname and language you choose, plus a randomly generated code that
  ...
  connection code in a cookie (for up to 30 days) and your nickname and language in your browser;
  the app stores the connection code and your last nickname and language on your phone, so they
  still show without a connection. After a server restart, the website and the app restore your
  last nickname and language automatically.
- In the app, games against the AI run on your phone. Their progress and score stay on your phone
  and are never sent to our server; they are erased when you leave the game or delete the app.
- Like any website, the server and our network provider Cloudflare keep connection logs (for example
  ...
  server's logs rotate. Cloudflare handles them under its own
  [privacy policy](https://www.cloudflare.com/privacypolicy/).
- Deleting data: clearing this site's data in your browser removes the connection code, nickname
